...
public class Miner {
    ...
    public static void assignAttrValue(HashMap<String, Data> ftable, String dataset, ArrayList<String> table, int idx) {
        String[] temp = Miner.getFrequency(table.get(idx));
        String[] s;

        for(int i = 0; i < temp.length; i++) {
            s = temp[i].split(" ");

            int yes = Miner.likelihood(dataset, s[0], "yes");
    ...
    public static void assignClass(HashMap<String, Data> ftable, ArrayList<String> table) {
        String[] temp = Miner.getFrequency(table.get(table.size()-1));
        String[] s;
    ...
    public static void printData(ArrayList<Vector4f> table) {
        for(int i = 0; i < table.size(); i++) {
            System.out.println(Arrays.toString(table.get(i).data.toArray()));
        }
